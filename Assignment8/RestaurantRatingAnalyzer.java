...
public class RestaurantRatingAnalyzer {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < ratingRanges.length - 1; i++) {
            ...
            ratingRangeCounts.put(range, 0);
            ratingRangeTotal.put(range, 0.0);
        }
        ...
        for (Restaurant restaurant : restaurants) {
            ...
            for (int i = 0; i < ratingRanges.length - 1; i++) {
                ...
                if (rating >= lowerBound && rating <= upperBound) {
                    ratingRangeCounts.put(range, ratingRangeCounts.get(range) + 1);
                    ratingRangeTotal.put(range, ratingRangeTotal.get(range) + rating);
                    break;
                }
            }
        }
